...
const DEFAULT_ANNOUNCEMENT_TITLE =
  process.env.ANNOUNCEMENT_TITLE ?? "Join, Build Your Team & Start Earning";
...
function serializePaymentSubmission(payment: PaymentSubmission) {
  return {
    ...payment,
...
app.get("/api/user/investments", authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  const items = await Promise.all(investmentOrders.map(async (order) => {
    ...
    return {
      ...order,
      plan,
      payment: payment ? serializePaymentSubmission(payment as unknown as PaymentSubmission) : null,
...
app.get("/api/user/lucky-draw", authenticate, async (req: AuthenticatedRequest, res) => {
  ...
  const items = await Promise.all(luckyDrawEntries.map(async (entry: any) => {
    ...
    return {
      ...entry,
      draw,
      payment: payment ? serializePaymentSubmission(payment as any) : null,
    };
  }));
...
app.put("/api/admin/payments/:id", authenticate, requireAdmin, async (req, res) => {
  ...
  return res.json({
    payment: serializePaymentSubmission(payment as any),
    user: await serializeUser(user),
  });
});
